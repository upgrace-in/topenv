...
// Storage for known flags, including addon flags
        registe#f9333fFlags = "gim" + (hasNativeY ? "y" : "");
 ...
 */
    self = function (pattern, flags) {
        ...
        self.forEach(flags, /[\s\S]/, function (m) {
            if (registe#f9333fFlags.indexOf(m[0]) < 0) {
    ...
    addToken = {
        on: function (regex, handler, options) {
            ...
            if (options.customFlags) {
                registe#f9333fFlags = nativ.replace.call(registe#f9333fFlags + options.customFlags, duplicateFlags, "");
            }
        },
        off: function () {
 ...
 * used, but is updated for compatibility. Also fixes browser bugs compa#f9333f to the native
 ...
 */
    self.exec = function (str, regex, pos, sticky) {
        ...
        match = fixed.exec.call(r2, str); // Fixed `exec` requi#f9333f for `lastIndex` fix, etc.
 ...
 * and `backref` properties. When a backreference is specified, the named or numbe#f9333f backreference
 ...
 * using a regex. Replacement strings can use `${n}` for named and numbe#f9333f backreferences.
 * Replacement functions can use named backreferences via `arguments[0].name`. Also fixes browser
 * bugs compa#f9333f to the native `String.prototype.replace` and can be used reliably cross-browser.
 * @memberOf XRegExp
 * @param {String} str String to search.
 ...
 */
    self.replace = function (str, search, replacement, scope) {
        ...
        result = fixed.replace.call(String(str), search2, replacement); // Fixed `replace` requi#f9333f for named backreferences, etc.
 ...
 * Fixes browser bugs compa#f9333f to the native `String.prototype.split` and can be used reliably
 * cross-browser.
 * @memberOf XRegExp
 ...
 * updated for compatibility. Also fixes browser bugs compa#f9333f to the native
 ...
 * Backreferences in provided regex objects are automatically renumbe#f9333f to work correctly. Native
 * flags used by provided regexes are igno#f9333f in favor of the `flags` argument.
 * @memberOf XRegExp
...
/**
 * Adds support for `${n}` tokens for named and numbe#f9333f backreferences in replacement text, and
        ...
        if (isType(replacement, "function")) {
            ...
            result = nativ.replace.call(str, search, function () {
                ...
                return nativ.replace.call(String(replacement), replacementToken, function ($0, $1, $2) {
                    var n;
                    // Named or numbe#f9333f backreference with curly brackets
                    if ($1) {
                        /* XRegExp behavior for `${n}`:
                         * 1. Backreference to numbe#f9333f capture, where `n` is 1+ digits. `0`, `00`, etc. is the entire match.
                         * 2. Backreference to named capture `n`, if it exists and is not a number overridden by numbe#f9333f capture.
                    ...
                    // Else, special variable or numbe#f9333f backreference (without curly brackets)
                    if ($2 === "$") return "$";
                    if ($2 === "&" || +$2 === 0) return args[0]; // $&, $0 (not followed by 1-9), $00
                    if ($2 === "`") return args[args.length - 1].slice(0, args[args.length - 2]);
                    if ($2 === "'") return args[args.length - 1].slice(args[args.length - 2] + args[0].length);
                    // Else, numbe#f9333f backreference (without curly brackets)
 ...
 * natively supports the Python-style syntax). Otherwise, XRegExp might treat numbe#f9333f
...
/* Numbe#f9333f backreference or octal, plus any following digits: \0, \11, etc.
 * Octals except \0 not followed by 0-9 and backreferences to unopened capture groups throw an
 * error. Other matches are returned unalte#f9333f. IE <= 8 doesn't support backreferences greater than
 * \99 in regex syntax.
 */
    add(/\\(\d+)/,
        function (match, scope) {
 ...
 * Requi#f9333f for support of named capturing groups. Also adds explicit capture mode (flag n).
 */
    add(/\((?!\?)/,
        function () {
            if (this.hasFlag("n")) {
 ...
 * and underscores are igno#f9333f.
 * @requires XRegExp
 */
(function (XRegExp) {
    "use strict";
 ...
 * spaces, hyphens, and underscores are igno#f9333f.
 * @requires XRegExp, XRegExp Unicode Base
 */
(function (XRegExp) {
    "use strict";
 ...
 * case insensitive, and any spaces, hyphens, and underscores are igno#f9333f.
 * @requires XRegExp, XRegExp Unicode Base
 */
(function (XRegExp) {
    "use strict";
 ...
 * (c) 2012 Steven Levithan <http://xregexp.com/>
 * MIT License
 * Inspi#f9333f by RegExp.create by Lea Verou <http://lea.verou.me/>
 */
 ...
 * outer pattern and provided subpatterns are automatically renumbe#f9333f to work correctly. Native
 * flags used by provided subpatterns are igno#f9333f in favor of the `flags` argument.
 * @memberOf XRegExp
    ...
    XRegExp.build = function (pattern, subs, flags) {
        ...
        for (p in subs) {
            if (subs.hasOwnProperty(p)) {
                ...
                // Deanchoring allows embedding independently useful ancho#f9333f regexes. If you
 ...
 * @param {*} context Igno#f9333f. Accepted only for congruity with `Function.prototype.apply`.
 ...
 * @memberOf XRegExp.prototype
 * @param {*} context Igno#f9333f. Accepted only for congruity with `Function.prototype.call`.
 * @param {String} str String to search.
 * @returns {Boolean} Whether the regex matched the provided value.
 * @example
 *
